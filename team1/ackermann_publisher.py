...
class DisparityExtender:
    ...
    MAX_ANGLE = 0.8
    SLOW_SPEED = 1.3
    MAX_DISTANCE_C = 0.975
    WHEELBASE_WIDTH=1.3#0.328
    ...
    def detect_red_blob(self, frame):
        ...
        for contour in contours:
            if cv2.contourArea(contour) > 50:  # Minimum blob size
    ...
    def after_initial_timer(self):
        self.started = True
        timer = Timer(4.5, self.reset_timer)
        timer.start()
    ...
    def _process_lidar(self, lidar_data):
        ...
        if max_value < (0.6 if not self.is_reversing else 0.95):
            speed = -0.75
